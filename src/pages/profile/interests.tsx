                    ...
                    {categories.map((category, index) => 
                        ((!category || userFollowings.includes(category))?(
                            <div className='relative'>
                                ...
                                {category ? (
                                    <button onClick={()=>handleRemoveInterest(category)} className={`absolute top-[-0.5rem] right-1 text-sm rounded-full bg-black text-white h-5 w-5 sm:h-6 sm:w-6 flex items-center justify-center ${(userFollowings.length == MIN_INTEREST_NEWS)?"hidden":""}`}>
