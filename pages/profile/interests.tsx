...
const interests = (props: Props) => {
    ...
    let tagDesign = "inline-block mr-3 mb-3 px-4 py-2 capitalize border border-primary rounded-[20px] transition-colors duration-300 hover:bg-primary hover:text-black";
    let interestDesign = "inline-block mr-3 mb-3 px-4 py-2 capitalize bg-primary border border-primary rounded-[20px] transition-colors duration-300 hover:bg-opacity-80";
    ...
    return (
        <div className="flex flex-col justify-center lg:justify-start items-center lg:items-start gap-y-4">
            <Box title="What are you interested in?">
                <p className='mb-5'>Select some topic you are interested in to help personalize your experience</p>
                <h1 className='text-xl font-bold text-left capitalize pb-4 font-semibold flex items-center mb-3'>Add Interests</h1>
                ...
                <div className='flex mt-5'>
                    {userFollowings.map((following, index) => (
                        <div className='relative'>
                            ...
                            <button onClick={()=>handleRemoveInterest(following)} className='absolute top-[-0.5rem] right-1 text-sm rounded-full bg-black text-white h-6 w-6 flex items-center justify-center'>
